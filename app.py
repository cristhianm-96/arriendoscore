...
def add_arrendatario(data, tipo="reportado"):
    token = str(uuid.uuid4())
    fecha = datetime.datetime.now().strftime("%Y-%m-%d")
    fecha_limite = (datetime.datetime.now() + datetime.timedelta(days=7)).strftime("%Y-%m-%d")
    estado = "activo" if tipo == "reportado" else "agregado"

    fila_completa = [data['email_propietario'], data['nombre'], data['cedula'], data['celular'], data['correo'], data['fecha_inicio'], data['fecha_fin'], data['meses_totales'], data['pagos_totales'], data['pagos_tiempo'], data['dias_atraso'], data['paz_salvo'], data['evidencias'], fecha, fecha_limite, estado, token, ""]

    sheet.worksheet("Arrendatarios").append_row(fila_completa)

    if tipo == "reportado": # Solo si es reporte va a Base_Universal y envía correo
        sheet.worksheet("Base_Universal").append_row(fila_completa)
        if data['correo']:
            enviar_correo_validacion_arrendatario(data['correo'], data['nombre'], token, data)
...
def dashboard():
    ...
    if request.method == "POST":
        ...
        elif 'btn_agregar_perfil' in request.form: # NUEVO: Solo agregar al perfil
            if arriendos_disponibles > 0:
                data = request.form.to_dict(); data['email_propietario'] = user['email']
                add_arrendatario(data, tipo="agregado")
                return redirect("/dashboard?msg=agregado")
            else:
                return redirect("/dashboard?msg=sin_cupos")

        elif 'btn_reportar' in request.form: # AHORA ES REPORTAR
            if arriendos_disponibles > 0:
                data = request.form.to_dict(); data['email_propietario'] = user['email']
                add_arrendatario(data, tipo="reportado")
    ...
    for i in arrendatarios:
        estado = i.get('estado','')
        if estado == "agregado": badge_class = "badge-agregado"
        elif "pendiente" in estado: badge_class = "badge-pendiente"
        elif estado == "activo": badge_class = "badge-activo"
        else: badge_class = "badge-disputa"

        filas += f"""<tr><td>{i.get('nombre','')}</td><td>{i.get('cedula','')}</td><td>{i.get('pagos_tiempo','')}/{i.get('meses_totales','')} meses</td><td>{i.get('dias_atraso','')} días</td><td><span class="badge {badge_class}">{estado}</span></td><td><form method="post" style="margin:0;"><input type="hidden" name="cedula_eliminar" value="{i.get('cedula','')}"><button name="btn_eliminar" class="btn-danger" onclick="return confirm('¿Eliminar de tu perfil?')">X</button></form></td></tr>"""

    alerta = ""
    if request.args.get('msg') == 'reportado': alerta = "<div style='padding:10px; background:#2ecc71; color:white; border-radius:8px; margin-bottom:15px;'>Arrendatario reportado. Ya quedó en Base Universal y se envió correo.</div>"
    if request.args.get('msg') == 'agregado': alerta = "<div style='padding:10px; background:#3498db; color:white; border-radius:8px; margin-bottom:15px;'>Arrendatario agregado a tu perfil.</div>"
