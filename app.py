...
def activar_usuario(email, codigo):
    ...
    if cell and str(ws.cell(cell.row, 9).value) == codigo: ws.update_cell(cell.row, 8, "activo"); ws.update_cell(cell.row, 9, ""); return True
    return False
...
def add_arrendatario(data):
    ...
    fila_completa = [data['email_propietario'], data['nombre'], data['cedula'], data['celular'], data['correo'], data['fecha_inicio'], data['fecha_fin'], data['meses_totales'], data['pagos_totales'], data['pagos_tiempo'], data['dias_atraso'], data['paz_salvo'], data['evidencias'], fecha, fecha_limite, "pendiente", token, ""]
...
def activar_arrendatario(token, accion, comentario=""):
    ...
    for idx, i in enumerate(arrendatarios):
        if str(i.get('token','')) == token:
            ...
            ws.update_cell(fila, 17, ""); return True
    return False

def delete_arrendatario(email_usuario, cedula):
    try: ws = sheet.worksheet("Arrendatarios"); arrendatarios = ws.get_all_records()
    for idx, r in enumerate(arrendatarios):
        if str(r.get('email_propietario','')) == email_usuario and str(r.get('cedula','')) == cedula: ws.delete_rows(idx + 2); return True
    except Exception as e: print("Error eliminando:", e)
    return False

@app.route("/")
def login(): return render_template_string(CSS + """<div class="page-wrapper"><div class="container"><img src="/logo.jpeg" class="logo"><h2>DatoArriendo</h2><form method="post" action="/login"><input name="email" type="email" placeholder="Email" required><input name="password" type="password" placeholder="Password" required><button>Entrar</button></form><a href="/registro">¿No tienes cuenta? Regístrate aquí</a></div></div>""")

@app.route("/registro", methods=["GET", "POST"])
def registro():
    if request.method == "GET": return render_template_string(CSS + """<div class="page-wrapper"><div class="container"><h2>Registro</h2><form method="post"><input name="nombre" placeholder="Nombre" required><input name="celular" placeholder="Celular" required><input name="email" type="email" placeholder="Email" required><input name="password" type="password" placeholder="Password" required><select name="rol" required><option value="">Seleccione...</option><option value="arrendador">Arrendador - 3</option><option value="inmobiliaria">Inmobiliaria - 50</option></select><button>Crear Cuenta</button></form></div></div>""")
    codigo = str(random.randint(100000, 999)); crear_usuario_temp(request.form['email'], request.form['password'], request.form['nombre'], request.form['celular'], request.form['rol'], codigo); enviar_codigo(request.form['email'], codigo, request.form['nombre']); session['email_temp'] = request.form['email']; return redirect("/validar")
...
def validar_arrendatario():
    token = request.args.get('token'); accion = request.args.get('accion'); comentario = request.args.get('comentario', '')
    if activar_arrendatario(token, accion, comentario): return render_template_string(CSS + """<div class="page-wrapper"><div class="container"><h2>Respuesta registrada</h2></div></div>""")
    ...
    if request.method == "POST":
        if 'btn_consultar' in request.form:
            ...
            try:
                ...
                if historiales: total_meses = sum(int(h.get('meses_totales',0)) for h in historiales); total_tiempo = sum(int(h.get('pagos_tiempo',0)) for h in historiales); mensaje_consulta = f"<div style='padding:12px; background:#27ae60; color:white; border-radius:8px;'><b>Historial:</b> {historiales[0].get('nombre')}<br>{total_tiempo} de {total_meses} meses a tiempo</div>"
            ...
            if delete_arrendatario(user['email'], request.form['cedula_eliminar']): return redirect("/dashboard")
