...
def add_solo_arrendatario(data):
    try:
        # SOLO GUARDA EN ARRENDATARIOS - 10 COLUMNAS. NO TOCA BASE_UNIVERSAL
        fila_arr = [
...
def dashboard():
    ...
    for i in reportados:
        link_contrato = i.get('evidencias_contrato','')
        btn_link = f"<a href='{link_contrato}' target='_blank' class='btn-link'>Ver Docs</a>" if link_contrato else "-"
        # CAMBIO: AGREGADAS FECHA INICIO Y FECHA FIN
        fila_html = f"""<tr><td>{i.get('nombre','')}</td><td>{i.get('cedula','')}</td><td>{i.get('fecha_inicio','')}</td><td>{i.get('fecha_fin','')}</td><td>{i.get('pagos_tiempo','0')}/{i.get('meses_totales','0')} meses</td><td>{i.get('dias_atraso','0')} días</td><td>{i.get('paz_salvo','')}</td><td>{btn_link}</td></tr>"""
        filas_reportados += fila_html

    alerta = ""
    if request.args.get('msg') == 'agregado': alerta = "<div style='padding:10px; background:#3498db; color:white; border-radius:8px; margin-bottom:15px;'>Arrendatario agregado correctamente a tu lista.</div>"
