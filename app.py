...
def get_arrendatarios(email_usuario):
    try:
        ...
        todos = ws.get_all_values() # CAMBIO 1: usar get_all_values para que no falle por headers
        headers = todos[0]
        data = []
        for row in todos[1:]:
            if len(row) < len(headers): row += [''] * (len(headers) - len(row))
            registro = dict(zip(headers, row))
            if str(registro.get('email_propietario','')).strip() == email_usuario:
...
def add_arrendatario(data, estado="agregado"):
    ...
    # CAMBIO 2: Orden exacto de columnas para que coincida con tu Sheet
    fila_completa = [
        ...
        data.get('fecha_fin',''), 
        data.get('meses_totales','0'), 
        data.get('pagos_totales','0'), 
        data.get('pagos_tiempo','0'), 
        data.get('dias_atraso','0'), 
        data.get('paz_salvo',''), 
        data.get('contrato_doc',''), 
        data.get('evidencias',''), 
        fecha, 
        fecha_limite, 
        estado, 
...
def delete_arrendatario(email_usuario, cedula):
    try:
        ...
        for idx, row in enumerate(todos[1:]):
            if len(row) >= 3 and str(row[0]).strip() == email_usuario and str(row[2]).strip() == cedula:
    ...
    if request.method == "POST":
        if 'btn_consultar' in request.form:
            ...
            if arriendos_disponibles > 0:
                ...
                add_arrendatario(data, estado="agregado") # CAMBIO 3: Agrega directo, sin correo
