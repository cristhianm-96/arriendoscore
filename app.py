...
def enviar_correo_validacion_arrendatario(destinatario, nombre, token, datos_reporte):
    def _enviar():
        try:
            ...
            cuerpo = f"""<html><body style="font-family:Arial;"><h2>Hola {nombre}</h2><p>Tu arrendador te ha registrado en <b>DatoArriendo</b></p><p><b>Periodo:</b> {datos_reporte.get('fecha_inicio')} a {datos_reporte.get('fecha_fin')}<br><b>Meses Totales:</b> {datos_reporte.get('meses_totales')}</p><p><a href='{link_aceptar}' style='padding:12px; background:#27ae60; color:white; text-decoration:none;'>Aceptar</a> <a href='{link_disputa}' style='padding:12px; background:#e74c3c; color:white; text-decoration:none;'>Disputar</a></p></body></html>"""
...
def add_arrendatario(data, estado="agregado"):
    ...
    # Quitamos los 4 campos: pagos_totales, pagos_tiempo, dias_atraso, paz_salvo
    fila_completa = [data['email_propietario'], data['nombre'], data['cedula'], data['celular'], data['correo'], data['fecha_inicio'], data['fecha_fin'], data['meses_totales'], "", data['contrato_doc'], fecha, fecha_limite, estado, token, ""]
    sheet.worksheet("Arrendatarios").append_row(fila_completa)
    sheet.worksheet("Base_Universal").append_row(fila_completa)
    if estado == "activo" and data['correo']:
...
def dashboard():
        ...
        if 'btn_consultar' in request.form:
            ...
            try:
                ...
                if historiales:
                    ...
                    mensaje_consulta = f"<div style='padding:12px; background:#27ae60; color:white; border-radius:8px; margin:10px 0;'><b>Historial encontrado:</b> {historiales[0].get('nombre')}<br>{total_meses} meses reportados.</div>"
    ...
    for i in arrendatarios:
        ...
        fila_html = f"""<tr><td>{i.get('nombre','')}</td><td>{i.get('cedula','')}</td><td>{i.get('meses_totales','')} meses</td><td><span class="badge {badge_class}">{estado}</span></td><td>"""
