...
def enviar_correo_validacion_arrendatario(destinatario, nombre, token, datos_reporte):
    def _enviar():
        try:
            ...
            cuerpo = f"""<html><body style="font-family:Arial;"><h2>Hola {nombre}</h2><p>Tu arrendador te ha registrado en <b>DatoArriendo</b></p><p><b>Periodo:</b> {datos_reporte.get('fecha_inicio')} a {datos_reporte.get('fecha_fin')}<br><b>Meses a Tiempo:</b> {datos_reporte.get('pagos_tiempo')} de {datos_reporte.get('meses_totales')}</p><p><a href='{link_aceptar}' style='padding:12px; background:#27ae60; color:white; text-decoration:none;'>Aceptar</a> <a href='{link_disputa}' style='padding:12px; background:#e74c3c; color:white; text-decoration:none;'>Disputar</a></p></body></html>"""
...
def get_arrendatarios(email_usuario):
    try:
        ...
        # Forzar que lea todas las filas aunque tenga vacíos
...
def reportar_arrendatario(email_propietario, cedula):
    ...
    for idx, i in enumerate(arrendatarios):
        if str(i.get('email_propietario','')) == email_propietario and str(i.get('cedula','')) == cedula:
            fila = idx + 2
            ws.update_cell(fila, 17, "activo") # columna 17 = estado
            token = i.get('token')
            if i.get('correo'):
...
def activar_arrendatario(token, accion, comentario=""):
    ...
    for idx, i in enumerate(arrendatarios):
        if str(i.get('token','')) == token:
            fila = idx + 2
            if accion == "aceptar": ws.update_cell(fila, 17, "activo")
            else: ws.update_cell(fila, 17, "en_disputa"); ws.update_cell(fila, 19, comentario)
            ws.update_cell(fila, 18, "")
            return True
    return False
...
def dashboard():
    ...
    # AQUI ESTA EL CAMBIO: NUEVA TABLA DE AGREGADOS
    filas_agregados = ""
    filas_reportados = ""
    for i in arrendatarios:
        estado = i.get('estado','')
        link_contrato = i.get('contrato_doc','')

        # SOLO PARA AGREGADOS: nueva estructura
        if estado == "agregado":
